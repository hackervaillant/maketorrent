...
def mktorrent(path, outfile, tracker=None, piecesize=2**18, private=False, magnet=False):
...
def main():
    ...
    parser.add_argument("path", help="Source file or folder.", type=validPath)
    parser.add_argument("outfile", help="The torrent file to produce.", type=argparse.FileType('wb'))
    parser.add_argument("-t", "--tracker", help="Tracker URL(S)", type=trackerUrl, nargs='*')
    parser.add_argument("-p", "--private", help="Make this torrent private to its tracker(s)", action='store_true')
    parser.add_argument("-m", "--magnet", help="Print magnet link before exit.", action='store_true')

    args = parser.parse_args()

    return mktorrent(**vars(args))
